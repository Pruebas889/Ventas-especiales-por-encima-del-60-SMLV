...
# Mes: Marzo 2026 
Write-Host "`n--- MARZO 2026 ---" -ForegroundColor Magenta
for ($dia = 1; $dia -le 31; $dia++) {
...
# Mes: Abril 2026 (hasta el 16 de abril - corte FIXED)
Write-Host "`n--- ABRIL 2026 (HASTA DIA 21 - CORTE FIXED) ---" -ForegroundColor Magenta
for ($dia = 1; $dia -le 30; $dia++) {
    $fecha = "2026-04-{0:D2}" -f $dia
    Write-Host "Cargando $fecha..." -NoNewline
    $response = Invoke-WebRequest -Uri "http://localhost:5050/api/cache/refresh/fixed/day?date=$fecha" -UseBasicParsing
    $result = $response.Content | ConvertFrom-Json
    Write-Host " $($result.result.rows) registros" -ForegroundColor Green
